...
- `dawn-300` `#ffa06f` — time markers and focus (7.7:1 on night-950)
...
`site/public/nightjar-social.webp` is a 1200 × 630 crop of the same original
source artwork. `site/public/apple-touch-icon.png` is a 180 px crop from that
artwork. Both were composed locally with ImageMagick 6.9.11 and add no new
third-party material. The sample audio is generated by
`examples/generate_sample.rs` from original synthesized tones and noise. Its
FLAC copy was encoded losslessly with FLAC 1.4.3.
